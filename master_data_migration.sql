-- Master Data Migration SQL
-- This script can be run safely multiple times
-- It will update existing data and insert new data as needed

-- Create master_data_categories table if it doesn't exist
...
CREATE INDEX IF NOT EXISTS idx_master_data_categories_type ON master_data_categories(type);
CREATE INDEX IF NOT EXISTS idx_master_data_categories_active ON master_data_categories(is_active);
CREATE INDEX IF NOT EXISTS idx_master_data_items_category ON master_data_items(category_id);
CREATE INDEX IF NOT EXISTS idx_master_data_items_active ON master_data_items(is_active);
CREATE INDEX IF NOT EXISTS idx_master_data_items_sort ON master_data_items(sort_order);

-- Insert/Update master data categories
...
-- Insert/Update Business Types data
...
-- Insert/Update Compliance Types data
INSERT INTO master_data_items (category_id, name, description, metadata, sort_order) 
SELECT 
  c.id,
  ct.name,
...
-- Insert/Update Document Categories data
INSERT INTO master_data_items (category_id, name, description, metadata, sort_order) 
SELECT 
  c.id,
  dc.name,
...
-- Insert/Update Authorities data
INSERT INTO master_data_items (category_id, name, description, metadata, sort_order) 
SELECT 
  c.id,
  auth.name,
...
-- Insert/Update Designations data
INSERT INTO master_data_items (category_id, name, description, metadata, sort_order) 
SELECT 
  c.id,
  des.name,
...
-- Insert/Update Internal Entities data
INSERT INTO master_data_items (category_id, name, description, metadata, sort_order) 
SELECT 
  c.id,
  ie.name,
...
DROP POLICY IF EXISTS "Allow read access to all authenticated users" ON master_data_categories;
CREATE POLICY "Allow read access to all authenticated users" ON master_data_categories
  FOR SELECT TO authenticated USING (true);

DROP POLICY IF EXISTS "Allow super_admin full access to categories" ON master_data_categories;
...
DROP TRIGGER IF EXISTS update_master_data_categories_updated_at ON master_data_categories;
CREATE TRIGGER update_master_data_categories_updated_at
    BEFORE UPDATE ON master_data_categories
    FOR EACH ROW
    EXECUTE FUNCTION update_updated_at_column();

DROP TRIGGER IF EXISTS update_master_data_items_updated_at ON master_data_items;
